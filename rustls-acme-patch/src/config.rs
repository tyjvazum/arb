...
///
/// The type parameters represent the error types for the certificate cache and
/// account cache.
...
impl AcmeConfig<Infallible, Infallible> {
    ...
    ///
    /// The new [AcmeConfig] instance will initially have no cache, and its type
    /// parameters for error types will be `Infallible` since the cache
    /// cannot return an error. The methods to set a cache will change the
    /// error types to match those returned by the supplied cache.
    ///
    /// ```rust
    ...
    /// [AcmeConfig::new] is not (yet) generic over the [AcmeConfig]'s type
    /// parameters. An uncached instance of [AcmeConfig] with particular
    /// type parameters can be created using [NoCache].
    ///
    /// ```rust
    /// # use rustls_acme::AcmeConfig;
...
impl<EC: 'static + Debug, EA: 'static + Debug> AcmeConfig<EC, EA> {
    ...
    pub fn cache_with_boxed_err<C: 'static + Cache>(
        self,
        cache: C,
    ) -> AcmeConfig<Box<dyn Debug>> {
        self.cache(BoxedErrCache::new(cache))
    }
    pub fn cache_option<C: 'static + Cache>(
        self,
        cache: Option<C>,
    ) -> AcmeConfig<C::EC, C::EA> {
        match cache {
    ...
    ///
    /// Specify supported protocol names in `alpn_protocols`, most preferred
    /// first. If emtpy (`Vec::new()`), we don't do ALPN.
    pub fn incoming<
