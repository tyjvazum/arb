use {
    super::*,
    arb::subcommand::wallet::send::Output,
};
...
fn inscriptions_can_be_sent() {
    ...
    let arb_server = TestServer::spawn_with_args(&rpc_server, &[]);
    arb_server.assert_response_regex(
...
#[test]
fn send_on_mainnnet_works_with_wallet_named_arb() {
    let rpc_server = test_bitcoincore_rpc::builder().build();
