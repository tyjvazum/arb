...
To search an `arb` wallet for sats with a name in `frequency.tsv`, use the
following [`arb`](https://github.com/tyjvazum/arb) command:

```
arb wallet sats --tsv frequency.tsv
```

This command requires the sat index, so `--index-sats` must be passed to arb
